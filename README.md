...
This replaces every template placeholder with your actual GitHub org and project name — the Go module path in `go.mod` and all Go source files, plus `CLAUDE.md`, `.golangci.yml`, `frontend/package.json`, `frontend/index.html`, `api/openapi.yaml`, the OTel service-name default (and its test), the deployment playbook's image names, and this README. The script warns if any placeholder survives, and `frontend/package-lock.json` is regenerated by the `npm install` step below.
